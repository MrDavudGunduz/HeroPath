      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <h4 className={cn('font-semibold text-white', task.completed && 'line-through')}>
              {task.title}
            </h4>
          ...
          {task.description && (
            <p className="mt-2 text-sm text-gray-300 whitespace-pre-wrap">{task.description}</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="secondary" size="sm" onClick={() => onRemove(task.id)}>
            Remove
          </Button>
        </div>
      </div>
